...
\subsection{Testing}
The purpose of writing a virtual machine for the produced machine code is twofold. The first is to combat the path divergence problem discussed in \cref{revSymEx}.
It also allows us to expect the execution of the produced code and check that is runs as expected. The compiler was tested `by inspection' - the produced CFG and the
...
and in doing so these tests achieve very high code coverage. In addition, the same examples were ran using the Valgrind debugging tool. This tool helps to detect memory leaks. More importantly, it an detect bugs that lead to undefined behaviour (these bugs might not have immediate obvious effects, but lead to problems down the line).

Finally, unit tests were carried out on sufficiently moduar parts of the code - i.e., finding loops, radix sort, building symbolic variables that represent our beliefs,
etc.
