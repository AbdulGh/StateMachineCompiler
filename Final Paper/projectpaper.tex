...
When we search for a variable, we first check the present scope. If it is not found there, we recursively search for it in the parent scope. If we end up running into a \texttt{nullptr}, we are trying to find a variable that has not been declared.
Otherwise we will find and return the `most recent' declaration of that variable. Variables can be tagged with various data - some necessary for finding errors (i.e. type) and some useful for warnings (i.e. whether that variable has been defined).\\
...
in line $3$ always gives an overflow warning, so long as the constant is greater than $1$.
 ...
 \subsubsection{Extrapolation} \label{extra}
...
\end{lstlisting}
\end{wrapfigure}
\textbf{Time:} 6ms.\\ \textbf{Maximum Resident Set Size:} $4448$KB.\\
This is an example that demonstrates the extrapolation of variable motion. $x$ is increasing away from the loop header condition, however
we know it is increasing towards the condition in line 3 and therefore will leave the loop. This is accepted as halting. Flipping the condition in line 1,
or turning line 2 into a decrement, result in programs that are still correctly classified as halting. Flipping the condition in line 3 results in a nonterminating
loop that is detected with the correct annotated trace given.

The method does make some mistakes. An invalid overflow warning is emitted for line 2. Also, changing the \texttt{x = -1} on line 3 into \texttt{x = 1}
results in a nonterminating loop that is not detected - the reason for this failure is given in the next section.
...
We also managed to detect a variety of bugs - overflows, out of bounds accesses, division by zero, and potential infinite loops. The method is especially good at detecting simple (yet common) errors - such as off by one errors, wrongly assuming that variables are in a certain range
...
itself can be used to further optimise code, for example by detecting infeasible code or useless operations.
...
one path that once entered can be left but will be revisited infinitely often - our method falsely accepts these cases as terminating. This is the case with the example given in
\cref{extra}. If $x$ is set to 1 we must infact leave that path in the next iteration, however we will eventually re-enter the path and repeat the cycle.
