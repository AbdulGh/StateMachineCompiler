...
\begin{abstract}
\par
{\bf Context/Background.} The field of program verification, although fraught with undecidability, has still seen great successes. Many techniques have been developed in order to
cope with different paradigms and requirements - for example, automated testing of 15K lines of video game code should differ from the formal verification of 600 lines
of safety-critical code.
\emph{Symbolic execution} is one such technique. It simulates the execution of a program using symbolic (rather than concrete) values for variables.
The development of a verifying compiler has been identified by Hoare as a `Grand Challenge for computing research' \cite{hoare}
...
Graph theoretic algorithms will be used in an attempt to determine the monotonicity of variables and certify termination (or 
infinite looping). The CFG will then be translated into machine code for a state-based virtual machine.
...
{\bf Conclusions.} In this paper we describe a bug detection method that can be built into the compilation process. This method quickly discoveres a range of errors.
\end{abstract}
...
Software has become one of the most inportant aspects of modern society.
...
This method, although powerful and simple, is hard to automate. One attempt \citep{colon} begins by generalising the idea of a ranking function to strongly connected components (SCS) in a control flow graph - where now the value of the ranking function never increases and strictly decreases on at least one edge in the SCS. Every potentially infinite loop must live in an SCS, and if we succeed in finding such a ranking function we know that an infinite execution cannot use any decreasing edges. We can then remove those edges - if the remainder of the component is still strongly connected, we repeat the procedure.
...
Graph searching algorithms are sometimes used to detect and remove unreachable code. This method only works with jumps and ignores everything else, and so leads
to an overapproximation of reachable code.

The search can be enriched with symbolic representations of variables. Each `probe' of the search will act as a possible execution of the program. As each
...
The first is `path explosion'. At every branch we may double the number of paths we must trace. While
...
The second are `complex constraints'. The path conditions generated may be intractable or even undecidable. Some more thorough program verifiers \cite{zhang, PNT}
...
Finally he discusses `path divergence'. Innacuracies will be inevitably introduced by modelling complex software with a more simple 
...
A similar approach to the solution presented here  `lasoo' approach discussed in \citep{gupta}. It begins by symbolically executing the program and finding paths that revisit some state. It then uses a constraint solver to find states that are recurrent.
...
This example is based on \textbf{A.1} in \cite{tra}, with the superficial addition of arrays (which was not supported by their method).

\subsubsection{Two variables (\cite{tra} A.8)}
...
\subsubsection{Relative change (\cite{tra} C.1)}
...
\subsubsection{Multiplication (\cite{tra} C.1)}
...
\subsubsection{The Ackermann function}
\begin{wrapfigure}{r}{0.4\textwidth}
\centering
\begin{lstlisting}[frame=tlrb,language=myLang]
function ack(double m, double n) double
{
 if (m <= 0) return n + 1;
  ...
  double a2 = call ack(m, n-1);
  return call ack(m-1, a2);
 }
}
\end{lstlisting}
\end{wrapfigure}
This function halts, as it is always called on strictly decreasing pairs (in the lexicographic ordering).
Changing the decrement in lines $3$ and $5$ into an an addition of a nonnegative constant is correctly seen to be nonterminating,
however the method fails to detect the same when line $7$ is modified.
...
\centering
\begin{lstlisting}[frame=tlrb,language=myLang]
function gcd(double m, double n)
{
 if (m == n) return m;
 else if (m > n)
  return call gcd(m-n, n);
 else return call gcd(m, n-m);
}
\end{lstlisting}
\end{wrapfigure}
\fi
