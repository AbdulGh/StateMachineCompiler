...
public:
    Loop(CFGNode* entry, CFGNode* last, std::map<CFGNode*, bool> nodeSet, unsigned long tailNumber, ControlFlowGraph& cfg);
    const std::map<CFGNode*, bool>& getNodes() {return nodes;}
    std::string getInfo();
